...
\]
When $f$ is high (homogeneous neighbourhood), pickup is unlikely --- the agent avoids breaking up existing clusters. When $f$ is low (mixed neighbourhood), pickup is more likely --- the agent targets isolated pellets. If pickup does not trigger, the agent performs a random walk (MOVE, TURN\_LEFT, or TURN\_RIGHT with equal probability).
